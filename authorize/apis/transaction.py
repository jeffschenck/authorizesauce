...
class TransactionAPI(object):
    ...
    def _add_params(self, params, credit_card=None, address=None, order=None,
                    email=None):
        if credit_card:
            params.update({
        ...
        if order:
            params.update({
                'x_invoice_num': order.invoice_num,
    ...
    def auth(self, amount, credit_card, address=None, order=None, email=None):
        amount = Decimal(str(amount)).quantize(Decimal('0.01'))
        params = self.base_params.copy()
        params = self._add_params(params, credit_card, address, order, email)
        params['x_type'] = 'AUTH_ONLY'
        params['x_amount'] = str(amount)
        return self._make_call(params)

    def capture(self, amount, credit_card, address=None, order=None,
                email=None):
        amount = Decimal(str(amount)).quantize(Decimal('0.01'))
        params = self.base_params.copy()
        params = self._add_params(params, credit_card, address, order, email)
